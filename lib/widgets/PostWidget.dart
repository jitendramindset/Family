...
//                  removeUserPost();
  ...
  removeUserPost()async{
    postsReference.doc(ownerId).collection("usersPosts").doc(postId)
        .get().then((document){
          if(document.exists)
            {
    ...
    QuerySnapshot commentsQuerySnapshot = await commentsReference.doc(postId)
  ...
  addLike(){
    ...
    if(!isNotPostOwner)
    {
    activityFeedReference.doc(ownerId).collection("feedItems").doc(postId).setData({
      "type": "Like",
